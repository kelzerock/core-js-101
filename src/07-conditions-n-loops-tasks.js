...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${leftBracket}${smallNum}, ${maxNum}${rightBracket}`;
}
...
function isCreditCardNumber(ccn) {
  // throw new Error('Not implemented');
  const newValue = ccn.toString();

  let nCheck = 0;
  let bEven = false;
  const arr = newValue.split('').reverse();
  arr.forEach((element) => {
    if (bEven) {
      if (element * 2 > 9) {
        nCheck += element * 2 - 9;
...
function getDigitalRoot(num) {
  ...
  while (newNum > 10) {
    ...
    newNum = str.split('').reduce((prev, curr) => prev + parseInt(curr, 10), 0);
  }
  return newNum;
}
